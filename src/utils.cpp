...
void drag3(Vec3 &vec, const std::string& label, float speed, float min, float max, float xSize, bool printLabel) {
	...
	ImGui::DragFloat(("##1" + label).c_str(), &vec.x, speed, min, max, "%.2f");
	ImGui::SameLine();
	ImGui::SetNextItemWidth(xSize / 3.f);
	ImGui::DragFloat(("##2" + label).c_str(), &vec.y, speed, min, max, "%.2f");
	ImGui::SameLine();
	ImGui::SetNextItemWidth(xSize / 3.f);
	ImGui::DragFloat(("##3" + label).c_str(), &vec.z, speed, min, max, "%.2f");
	ImGui::SameLine();
	if (printLabel)
